...
// Ingen WiFi/cloud under lokal test
SYSTEM_MODE(MANUAL);
...
// Indstillinger
// =========================
// Start lavt for at undgå RAM-problemer.
// Når det compiler og virker, kan RECORD_MS prøves hævet til 1000.
...
static double trim_rms_values[MAX_TRIM_FRAMES];
...
static void feature_vector_to_float_array(const Feature2Vector *fv, float features[CHORD_NUM_FEATURES])
{
    int idx = 0;
    ...
    for (int i = 0; i < F2_NUM_CHROMA; i++)
    {
        features[idx++] = (float)fv->chroma_cens[i];
    }

    for (int i = 0; i < F2_NUM_TONNETZ; i++)
    {
        features[idx++] = (float)fv->tonnetz[i];
    }

    if (idx != CHORD_NUM_FEATURES)
    {
        Serial.print("WARNING: feature count mismatch. idx=");
        Serial.print(idx);
        Serial.print(", CHORD_NUM_FEATURES=");
        Serial.println(CHORD_NUM_FEATURES);
    }
}


static void print_features(const float features[CHORD_NUM_FEATURES])
{
#if PRINT_FEATURES
    ...
    for (int i = 0; i < CHORD_NUM_FEATURES; i++)
    {
        Serial.print(i);
        Serial.print(" ");
        Serial.print(CHORD_FEATURE_NAMES[i]);
        Serial.print(": ");
...
// =========================
static int predict_rf(const float features[CHORD_NUM_FEATURES], unsigned long *time_us)
{
...
static void classify_audio()
{
    ...
    int rf_pred = predict_rf(features, &rf_time);

    Serial.println("=================================");
    Serial.println("Prediction result");

    if (rf_pred >= 0 && rf_pred < CHORD_NUM_CLASSES)
    {
        Serial.print("RF predicted class: ");
        Serial.println(rf_pred);

        Serial.print("RF predicted chord: ");
        Serial.println(CHORD_LABELS[rf_pred]);
    }
    else
    {
        Serial.print("RF invalid prediction: ");
        Serial.println(rf_pred);
    }

    Serial.print("RF inference time: ");
...
void setup()
{
    ...
    Serial.println("PDM microphone -> features -> RF model");
    Serial.println("KNN is disabled to reduce RAM");
    Serial.println("=================================");
    ...
    Serial.print("CHORD_NUM_CLASSES: ");
    Serial.println(CHORD_NUM_CLASSES);
